	...
	std::vector<TestCase> test_cases = {
		//	// Basic Methods 
		{
			...
			Request("GET", "/index.html", "", "HTTP/1.1",
					{{"host", "localhost"}}, "", StatusCode::Ok)
		},
		{
			"Simple POST with body",
			"POST /submit HTTP/1.1\r\nHost: localhost\r\nContent-Length: 11\r\n\r\nhello world",
			Request("POST", "/submit", "", "HTTP/1.1",
					{{"host", "localhost"}, {"content-length", "11"}}, "hello world", StatusCode::Ok)
		},
		{
			"DELETE request",
			"DELETE /resource/42 HTTP/1.1\r\nHost: localhost\r\n\r\n",
			Request("DELETE", "/resource/42", "", "HTTP/1.1",
					{{"host", "localhost"}}, "", StatusCode::Ok)
		},
		// ─── Query Strings ───────────────────────────────────────────────
		{
		...
		},
		{
			...
			Request("GET", "/search", "q=hello&page=2&sort=asc", "HTTP/1.1",
					{{"host", "localhost"}}, "", StatusCode::Ok)
		},
		{
		...
		// ─── POST edge cases ─────────────────────────────────────────────
		{
			...
			"POST /submit HTTP/1.1\r\nHost: localhost\r\nContent-Length: 0\r\n\r\n",
			Request("POST", "/submit", "", "HTTP/1.1",
					{{"host", "localhost"}, {"content-length", "0"}}, "", StatusCode::Ok)
		},
		...
		// ─── Headers edge cases ──────────────────────────────────────────
		{
			...
			Request("GET", "/", "", "HTTP/1.1",
					{{"host", "localhost"}, {"accept", "text/html, application/json"}}, "", StatusCode::Ok)
		},
		{
		...
		// ─── Chunked body ────────────────────────────────────────────────
		{
			...
			Request("POST", "/upload", "", "HTTP/1.1",
					{{"host", "localhost"}, {"transfer-encoding", "chunked"}}, "hello", StatusCode::Ok)
		},
		{
			"Chunked transfer encoding multiple chunks",
			"POST /upload HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n5\r\nhello\r\n6\r\n world\r\n0\r\n\r\n",
			Request("POST", "/upload", "", "HTTP/1.1",
					{{"host", "localhost"}, {"transfer-encoding", "chunked"}}, "hello world", StatusCode::Ok)
		},
		{
		...
		},
		{
			...
			"POST /upload HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n5\r\nhello\r\n0\r\nX-Checksum: abc123\r\n\r\n",
			Request("POST", "/upload", "", "HTTP/1.1",
					{{"host", "localhost"}, {"transfer-encoding", "chunked"}, {"x-checksum", "abc123"}}, "hello", StatusCode::Ok)
		},
...
std::vector<TestCase> generate_bad_test_cases(void)
{
	std::vector<TestCase> test_cases = {
		...
		},
		{
			...
			Request("PUT", "/resource/42", "", "HTTP/1.1",
					{{"host", "localhost"}, {"content-length", "4"}}, "data", StatusCode::NotImplemented)
		},
		// ─── Body errors ─────────────────────────────────────────────────
		{
