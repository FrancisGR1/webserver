...
EventManager::EventManager(size_t max_events, ConnectionPool &connections)
    : m_epoll_fd(-1), m_epoll_events_buffer(new epoll_event[max_events]),
      m_current_event(0), m_n_events(-1), m_max_events(max_events),
...
EventManager::~EventManager() {
  ...
  for (std::list<EventAction *>::iterator it = m_events.begin();
       it != m_events.end(); ++it) {
    ::close((*it)->fd);
    delete *it;
  }
...
int EventManager::wait() {
  ...
  m_n_events = epoll_wait(m_epoll_fd, m_epoll_events_buffer,
                          constants::max_events, constants::epoll_timeout);
  if (m_n_events == -1)
    ...
    if (Logger::level() <= Log::Debug) {
      ...
      for (size_t i = 0; i < static_cast<size_t>(m_n_events); ++i) {
        EventAction *ea =
            static_cast<EventAction *>(m_epoll_events_buffer[i].data.ptr);
        EventAction *found = find(ea);
        if (found)
          Logger::debug("EventManager: event=%d: '%s'", i,
                        found->str().c_str());
        else
...
bool EventManager::next(EventAction *&out) {
  ...
  while (m_current_event < static_cast<size_t>(m_n_events)) {
    EventAction *ea = static_cast<EventAction *>(
        m_epoll_events_buffer[m_current_event++].data.ptr);
...
const EventAction *EventManager::get_event(size_t index) const {
  ...
  REQUIRE(m_epoll_events_buffer[index].data.ptr != NULL,
          "data.ptr must never be null!");

  EventAction *ea =
      static_cast<EventAction *>(m_epoll_events_buffer[index].data.ptr);
  if (ea == NULL) {
    Logger::warn("EventManager: found stale event");
    return NULL;
  } else {
    Logger::debug("EventManager: Getting idx=%d: '%s'", index,
                  ea->str().c_str());
  }
...
int EventManager::to_epoll_event(const EventAction &ea) {
  ...
  case EventAction::WantRead: {
    ...
    Logger::trace("EventManager: set fd='%zu' to '%s'", ea.fd,
  ...
  if (ret == -1) {
    Logger::error("EventManager: epoll_ctl() error: says: '%s'",
                  strerror(errno));
  }
...
int EventManager::remove(const EventAction &ea) {
  ...
  for (std::list<EventAction *>::iterator it = m_events.begin();
    ...
    if (ea.fd == e->fd) {
      ...
      if (e->type == EventAction::ClientSocket ||
          e->type == EventAction::ServerSocket) {
        ret = ::close(e->fd);
        if (e->type == EventAction::ClientSocket && e->conn != NULL) {
          Logger::trace("EventManager: delete connection[id=%zu] fd='%zu'",
                        ea.conn->id(), ea.fd);
...
EventAction *EventManager::find(EventAction *&ea) const {
  ...
  for (std::list<EventAction *>::const_iterator it = m_events.begin();
       it != m_events.end(); ++it) {
    if (ea == *it) {
      Logger::trace("EventManager: found fd='%d' in events", ea->fd);
      if ((*it)->conn)
        Logger::trace("EventManager: found connection: '%lld'",
                      (*it)->conn->id());
      else
...
EventAction *EventManager::exists(int event_fd) const {
  ...
  for (std::list<EventAction *>::const_iterator it = m_events.begin();
    ...
    if (event_fd == (*it)->fd) {
      if ((*it)->conn)
        Logger::trace("EventManager: found connection: id=%lld,fd=%d",
                      (*it)->conn->id(), (*it)->fd);
      else
        Logger::trace("EventManager: server socket: '%d'", (*it)->fd);
      return *it;
...
void EventManager::flush_pending_deletes(void) {
  for (std::list<EventAction *>::iterator it = m_pending_deletes.begin();
       it != m_pending_deletes.end(); ++it)
    delete *it;
