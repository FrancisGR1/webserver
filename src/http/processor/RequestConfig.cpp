...
const ServiceConfig& RequestConfig::service() const
{
    return m_service;
}

const Path& RequestConfig::path() const
{
    return m_resolved_path;
}

bool RequestConfig::has_location() const
{
    return m_location != NULL;
}

const LocationConfig* RequestConfig::location() const
{
    return m_location;
}
...
bool RequestConfig::is_cgi() const
{
    return m_resolved_path.is_cgi;
}
...
{
    if (m_location)
        return utils::contains(m_location->methods, method);
    return (false);
}
