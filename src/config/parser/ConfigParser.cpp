...
void ConfigParser::expect(const Token& t, Token::Type expected)
{
    if (t.type != expected)
    {
        throw std::runtime_error(
            "Error - expected: " + token_type_to_literal(expected) + ", found: " + t.to_literal() +
            " - str: " + t.value);
    }
}
...
void ConfigParser::expect(const Token& t, char expected, size_t pos)
{
    ...
    {
        throw std::runtime_error(
            "Error - expected: " + std::string(1, expected) + ", found: " + t.to_literal() + " - str: " + t.value);
    }
}
...
void ConfigParser::expect(const Token& t, Token::Type expected_type, char expected_ch, size_t pos)
{
    ...
    {
        throw std::runtime_error(
            "Error - expected: " + token_type_to_literal(expected_type) + ", found: " + t.to_literal() +
            " - str: " + t.value);
    }
}
