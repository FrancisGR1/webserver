...
void ServiceConfig::set(Directive& directive)
{
    ...
    {
        case Token::DirectiveServerName: server_name = directive.args.at(0); break;
        case Token::DirectiveListen: listeners.push_back(directive.listen); break;
        ...
        case Token::DirectiveMaxBodySize: max_body_size = atoi(directive.args.at(0).c_str()); break;
        default: throw std::runtime_error("Invalid: can't set " + token_type_to_literal(directive.type) + "\n");
    }
}
...
std::ostream& operator<<(std::ostream& os, const ServiceConfig& service)
{
    ...
    for (std::map<std::string, LocationConfig>::const_iterator it = service.locations.begin();
         it != service.locations.end();
         it++)
    {
    ...
    os << "Error pages:\n";
    for (std::map<size_t, std::string>::const_iterator it = service.error_pages.begin();
         it != service.error_pages.end();
         it++)
    {
